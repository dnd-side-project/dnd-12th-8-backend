...
public class MyPageController {
    ...
    @Operation(summary = "마이페이지 피드백 참여 프로젝트 조회", description = "사용자가 피드백 참여한 프로젝트 조회")
    @GetMapping("/participate-project")
    public ResponseEntity<ApiResponse<List<MyPageProjectResponseDto>>> getParticipateProject(
      @AuthenticationPrincipal OAuthUserDetails oAuthUserDetails) {
        List<MyPageProjectResponseDto> myPageProjectResponseDtoList = myPageService.getMyPageParticipateProjectList(
          oAuthUserDetails.getMemberId());
        return ResponseEntity.ok(
          new ApiResponse<>(HttpStatus.OK.value(), "조회 성공", myPageProjectResponseDtoList));
    }
